...
def print_command(network, namelist):
    ...
    sorted_names = sorted(network.keys())
...
def common_friends_command(network, namelist):
    ...
    # Initialized a list for common friends, so they can later be printed out in alphabetical order.
    common_friends = []
    ...
    # Loop through friends of name1 & name2 and adds common friends to the common_friends list.
    for friend_name in name1_friends:
        if friend_name in name2_friends:
            common_friends.append(friend_name)

    if len(common_friends) == 0:
        print(f"{name1} and {name2} have no common friends.")
    else:
        # Print out common friends in alphabetical order.
        for common_friend in sorted(common_friends):
            print(common_friend)
